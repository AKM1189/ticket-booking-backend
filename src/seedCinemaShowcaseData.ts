...
type SeedMovie = {
  ...
  casts: string[];
  trailerId: string;
  statusGroup: "nowShowing" | "ticketAvailable" | "comingSoon";
};
...
const SEAT_TYPE_NAMES = ["Standard", "Prime", "Recliner"] as const;
const BRAND_NAME = "Aurora Cinemas";

const seatTypeSeeds: SeedSeatType[] = [
...
async function main() {
  ...
  try {
    ...
    const seatTypeRepo = AppDataSource.getRepository(SeatType);
    const theatreRepo = AppDataSource.getRepository(Theatre);
    const screenRepo = AppDataSource.getRepository(Screen);
    const screenSeatTypeRepo = AppDataSource.getRepository(ScreenSeatType);
    const movieRepo = AppDataSource.getRepository(Movie);
    const scheduleRepo = AppDataSource.getRepository(Schedule);
    const scheduleSeatTypeRepo = AppDataSource.getRepository(ScheduleSeatType);
    ...
    const screenReferences = await seedTheatresAndScreens(
      theatreRepo,
      screenRepo,
      screenSeatTypeRepo,
      seatTypeMap,
...
async function seedSeatTypes(repo: Repository<SeatType>) {
...
async function seedGenres(repo: Repository<Genre>) {
...
async function seedTheatresAndScreens(
  theatreRepo: Repository<Theatre>,
  screenRepo: Repository<Screen>,
  screenSeatTypeRepo: Repository<ScreenSeatType>,
  seatTypeMap: Map<string, SeatType>,
) {
  const screenReferences: ScreenReference[] = [];
  ...
  for (const theatreSeed of theatreSeeds) {
    ...
    for (const screenSeed of theatreSeed.screens) {
      const capacity =
        SCREEN_ROWS * SCREEN_COLS - screenSeed.disabledSeats.length;
      ...
      await syncScreenSeatTypes(
        screenSeatTypeRepo,
        screen,
        screenSeed.disabledSeats,
        seatTypeMap,
      );
...
async function syncScreenSeatTypes(
  screenSeatTypeRepo: Repository<ScreenSeatType>,
  screen: Screen,
  disabledSeats: string[],
  seatTypeMap: Map<string, SeatType>,
  ...
  const existingBySeatTypeId = new Map(
    existing.map((item) => [item.seatType.id, item]),
  );
...
async function clearSeedSchedules(
  seededTitles: string[],
  movieRepo: Repository<Movie>,
  scheduleRepo: Repository<Schedule>,
  scheduleSeatTypeRepo: Repository<ScheduleSeatType>,
) {
  ...
  const schedules = await scheduleRepo.find({
    where: {
      movie: { id: In(movies.map((movie) => movie.id)) },
    },
    relations: ["bookings"],
  });
  ...
  if (protectedSchedules.length) {
    throw new Error(
      `Cannot reseed schedules because ${protectedSchedules.length} seeded schedules already have bookings.`,
    );
  }
...
async function seedMovies(
  ...
  for (let index = 0; index < movieSeeds.length; index += 1) {
    ...
    const poster = await upsertImage(
      imageRepo,
      buildImageUrl(seed.title, "Poster", posterBg, posterFg, 600, 900),
    );
    const photos = await Promise.all(
      ["Scene 1", "Scene 2", "Scene 3"].map((label, photoIndex) =>
        upsertImage(
          imageRepo,
          buildImageUrl(
            `${seed.title} ${label}`,
            `Gallery ${photoIndex + 1}`,
            posterFg,
            posterBg,
            1280,
            720,
...
async function syncMovieRelations(
  ...
  if (relation === "casts") {
    ...
    if (nextIds.length) {
      ...
      await movieRepo.query(
        `INSERT INTO cast_movies_movie (castId, movieId) VALUES ${valuesSql}`,
        params,
      );
      const reverseParams = nextIds.flatMap((castId) => [movieId, castId]);
      await movieRepo.query(
        `INSERT INTO movie_casts_cast (movieId, castId) VALUES ${valuesSql}`,
        reverseParams,
      );
    }
  ...
  const relationQuery = movieRepo
    ...
    .of(movieId);
  const current = (await relationQuery.loadMany()) as Array<{ id: number }>;
...
async function seedSchedules(
  ...
  const nowShowingMovies = movieSeeds.filter(
    (movie) => movie.statusGroup === "nowShowing",
  );
  const ticketAvailableMovies = movieSeeds.filter(
    (movie) => movie.statusGroup === "ticketAvailable",
  );
  ...
  for (let index = 0; index < nowShowingMovies.length; index += 1) {
    ...
    await persistSchedulesForMovie(
      movie,
      plans,
      movieSeed,
      seatTypeMap,
...
async function persistSchedulesForMovie(
  movie: Movie,
  plans: SchedulePlan[],
  movieSeed: SeedMovie,
  seatTypeMap: Map<string, SeatType>,
...
async function syncScheduleSeatTypes(
  schedule: Schedule,
  screen: Screen,
  seatTypeMap: Map<string, SeatType>,
  scheduleSeatTypeRepo: Repository<ScheduleSeatType>,
  ...
  for (const seatTypeName of SEAT_TYPE_NAMES) {
    ...
    await scheduleSeatTypeRepo.save(
      scheduleSeatTypeRepo.create({
        schedule,
        seatType,
        price: toMoney(
          Number(seatType.price) *
...
async function refreshDerivedCounts(
...
async function refreshMovieStatuses(movieRepo: Repository<Movie>) {
  const seededMovies = await movieRepo.find({
    where: { title: In(movieSeeds.map((movie) => movie.title)) },
    relations: ["schedules"],
  });

  const releaseDateOverrides = new Map(
    movieSeeds
  ...
  for (const movie of seededMovies) {
    if (movie.schedules?.length) {
      const earliestShow = movie.schedules
        .map((schedule) => dayjs(schedule.showDate))
...
async function buildSummary(
  ...
  const schedules = await scheduleRepo.find({
    where: { movie: { title: In(seededTitles) } },
    relations: ["movie", "theatre", "screen"],
  });

  const statusBreakdown = seededMovies.reduce<Record<string, number>>(
    (acc, movie) => {
...
function buildTodayTimeCandidates(index: number) {
  const now = dayjs();
  const candidate = now
...
function allocateSlot(
  ...
  for (const dayOffset of dayOffsets) {
    const showDate = dayjs().add(dayOffset, "day").format("YYYY-MM-DD");
...
function buildSeatAssignments(disabledSeats: string[]) {
  ...
  const assignments: Record<(typeof SEAT_TYPE_NAMES)[number], string[]> = {
    Standard: [],
    Prime: [],
    Recliner: [],
  };
  ...
  for (let rowIndex = 0; rowIndex < SCREEN_ROWS; rowIndex += 1) {
    ...
    const seatTypeName =
      rowIndex <= 3 ? "Standard" : rowIndex <= 5 ? "Prime" : "Recliner";
...
function getScheduleMultiplier(experience: string[]) {
...
function resolveScheduleStatus(
  ...
  const now = dayjs();
  const start = dayjs(`${showDate} ${showTime}`);
...
function resolveMovieStatus(movie: Movie) {
  const today = dayjs();
  ...
  const showDates = movie.schedules.map((schedule) => dayjs(schedule.showDate));
  ...
  if (hasFuture) {
    return MovieStatus.ticketAvailable;
  }
...
async function upsertImage(repo: Repository<Image>, url: string) {
